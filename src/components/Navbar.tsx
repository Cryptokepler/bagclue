...
export default function Navbar() {
  ...
  return (
    <nav className={`transition-all duration-300 bg-white border-b border-gray-100`}>
      <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(l => (
            <Link key={l.href} href={l.href} className="text-sm tracking-widest uppercase text-gray-600 hover:text-[#E85A9A] transition-colors">
              {l.label}
            </Link>
          ))}
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white/98 backdrop-blur-md border-t border-gray-100 animate-fade-in">
          <div className="px-6 py-6 flex flex-col gap-4">
            ...
            <a href="https://ig.me/m/salebybagcluemx" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center justify-center gap-2 bg-[#E85A9A] text-white px-6 py-3 text-sm tracking-widest uppercase font-medium hover:bg-[#EC5C9F] transition-colors">
